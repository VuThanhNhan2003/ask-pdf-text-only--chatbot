...
def _build_query_from_chunk(chunk: NormalizedChunk) -> str:
    ...
    if topic and keyword_text:
        return f"In the document, what is the main content about '{topic}'? (related: {keyword_text})"
    if topic:
        return f"In the document, what is the main content about '{topic}'?"

    sentence = _first_sentence(chunk.text, max_len=120)
    if sentence:
        return f"Briefly explain the main idea of the following passage: {sentence}"
    return "What is the main content of this section?"
...
def _build_synthesis_prompt(answer_text: str, query_type: str) -> str:
    ...
    if qt == "enumeration":
        style_hint = (
            "Connect items into prose using commas, semicolons, or conjunctions like "
            "'including', 'additionally', 'finally'. Do not omit any item."
        )
    elif qt == "compare_contrast":
        style_hint = (
            "Present similarities/differences in a coherent paragraph, using 'meanwhile', "
            "'in contrast', 'similarly' as transitions. Do not use tables or lists."
        )
    elif qt == "reasoning":
        style_hint = (
            "State the conclusion first, then present reasons using conjunctions like "
            "'because', 'therefore', 'since'. Write in complete sentences with clear logic."
        )
    else:
        style_hint = (
            "Write in natural flowing prose, using appropriate conjunctions and transitional sentences. "
            "Preserve all technical terms, mathematical symbols, formulas, and figures."
        )

    return f"""Rewrite the content below as a coherent flowing paragraph, without bullets, dashes, or numbered items.

Mandatory requirements:
- Preserve ALL information, do not add or omit any point.
- Do not use bullet symbols (-, *, •), dashes, or numbered items (1., 2., a., b.).
- {style_hint}
- Return only the rewritten paragraph, without any preamble, heading, or notes.
...
def _format_hint_by_query_type(query_type: str) -> str:
    """
    Soft style hints per query type — guidance for prose writing, not bullets.
    """
    qt = (query_type or "").strip().lower()
    if qt == "definition":
        return (
            "- Definition: State the core concept and key characteristics in a coherent paragraph.\n"
            "- Prioritize accuracy and completeness — do not write as a list."
        )
    if qt == "compare_contrast":
        return (
            "- Compare: Present similarities and differences in prose, using 'meanwhile', 'in contrast', 'similarly' as transitions.\n"
            "- Do not write as a table or list."
        )
    if qt == "enumeration":
        return (
            "- Enumerate: Instead of numbered items, connect them with commas, semicolons, or conjunctions ('including', 'additionally', 'finally').\n"
            "- Ensure no important point is omitted."
        )
    if qt == "reasoning":
        return (
            "- Reasoning: State the conclusion first, then explain reasons using logically connected sentences.\n"
            "- Use conjunctions like 'because', 'therefore', 'since' instead of numbered items."
        )
    if qt == "role":
        return (
            "- Role/function: Present all functional aspects in a paragraph, without using lists.\n"
            "- May elaborate if the context provides additional useful information."
        )
    return "- Answer clearly in prose, stay close to the context, prioritize accuracy and completeness of main points."


def _estimate_reference_complexity(reference: str, query_type: str = "") -> Dict[str, Any]:
    """
    Analyze the reference answer to estimate the expected complexity of the response.

    Returns a dict with the following fields:
      - words        : number of words in reference
      - key_points   : estimated number of main points
      - length_tier  : "short" | "medium" | "long" | "very_long"
      - target_range : length suggestion string for the prompt (e.g. "1-2 sentences", "4-6 points")
    """
    text = (reference or "").strip()

    # If reference is empty or a placeholder, treat as a short sentence.
    if not text or _is_placeholder_reference(text):
        return {
            "words": 0,
            "key_points": 1,
            "length_tier": "short",
            "target_range": "1-2 short sentences",
        }

    words = len(re.findall(r"\w+", text, flags=re.UNICODE))

    # Count bullet/numbered items in the reference (if any).
    ...
    # key_points = max of bullet items and sentences, but not exceeding 8.
    key_points = max(bullet_items, sentences, 1)
    key_points = min(key_points, 8)

    qt = (query_type or "").strip().lower()

    # Determine length_tier based on word count, adjusted by query_type.
    if words <= 40:
        tier = "short"
    elif words <= 100:
        tier = "medium"
    ...
    # Enumeration/compare_contrast typically needs more points even if reference is short.
    if qt in {"enumeration", "compare_contrast"} and tier == "short":
        tier = "medium"

    # Mapping tier → target_range for the prompt.
    range_map = {
        "short":     "1-2 short sentences",
        "medium":    f"about {min(key_points + 1, 4)} main points",
        "long":      f"about {min(key_points, 6)} points — complete, no omissions",
        "very_long": f"all main points (about {key_points}) — prioritize completeness over brevity",
    }
...
def _build_adaptive_length_guidance(
    ...
    """
    Generate length guidance based on the reference answer's complexity.

    Logic:
    - short     → always concise regardless of concise_mode (True or False).
    - medium    → concise_mode is slightly stricter.
    - long / very_long → prioritize completeness, ignore concise_mode.
    """
    if ref_complexity is None:
        # No reference → use previous behavior.
        if concise_mode:
            return "Answer concisely and on-point — no rambling, no unnecessary explanations."
        return "Answer all main points — may elaborate further if context allows."

    tier = ref_complexity.get("length_tier", "medium")
    target = ref_complexity.get("target_range", "2-3 points")

    if tier == "short":
        return f"Answer briefly: {target}. No elaboration, no unnecessary explanations."

    if tier == "medium":
        if concise_mode:
            return f"Answer concisely: {target}. On-point, no rambling."
        return f"Answer fully: {target}. Cover all important points."

    if tier == "long":
        # For complex questions, ignore concise_mode to avoid cutting important points.
        return (
            f"Answer fully: {target}. "
            "Ensure ALL important points are included — avoid omissions, but do not add ideas outside the context."
        )

    # very_long
    return (
        f"Answer comprehensively: {target}. "
        "Prioritize completeness over brevity — this question requires extensive information."
    )
...
def build_eval_answer_prompt(
    ...
    - reference_complexity: output of _estimate_reference_complexity().
      If None, falls back to previous behavior (concise_mode flag).
    """
    ...
    return f"""You are an academic assistant. Answer the question based on the provided context.
...
Requirements:
1. Only use information available in the context — do not add outside knowledge.
2. {length_guidance}
3. Write in coherent flowing prose — do not use dashes, bullets, numbered items, or enumeration symbols.
4. Preserve technical terms, formulas, and figures if present in the context.
5. Do not use unnecessary preamble or meta-text (e.g., "Based on the context, I would like to answer...").
6. If the context does not have sufficient information: respond with "Insufficient data in the context".
{style_hint}
