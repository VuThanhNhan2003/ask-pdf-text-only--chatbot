...
(function () {
  ...
  class ChatbotWidget {
    constructor(cfg = {}) {
      ...
      this._streamFramePending = false;

      this._injectStyles();
      this._buildDOM();
      this._bindEvents();
      this._loadSubjects();
      // Preload render dependencies to minimize first-response latency.
      this._ensureLibsReady();
    }
    ...
    _ensureLibsReady() {
      if (this.libsReady) return Promise.resolve();
      if (!this._libPromise) {
        this._libPromise = ensureLibs()
          .then(() => {
            this.libsReady = true;
            this._rerenderExistingBotBubbles();
          })
          .catch(() => {
            // Keep widget functional in plain-text mode if CDN is unavailable.
            this.libsReady = false;
          });
      }
    ...
    _rerenderExistingBotBubbles() {
      if (!this.$messages || !this.libsReady) return;
      this.$messages.querySelectorAll('.rcw-row.bot .rcw-bubble').forEach(bubble => {
        if (typeof bubble._rcwRaw !== 'string') return;
        this._renderBotBubble(bubble, bubble._rcwRaw, { skipScroll: true });
      });
      this._scrollBottom();
    }
    ...
    _injectStyles() {
      ...
      style.textContent = `
        ...
        #rcw-root {
          ...
          --p-light: color-mix(in srgb, ${p} 14%, #fff);
          --radius: 22px;
          --radius-sm: 12px;
          ...
          --msg-bot-text: #0f172a;
          --input-bg: #ffffff;
          --header-bg: linear-gradient(135deg, color-mix(in srgb, var(--p) 84%, #0b1324), color-mix(in srgb, var(--p) 72%, #10253f));
          --header-text: #ffffff;
          --code-bg: #f6f8fb;
          --code-header-bg: #edf2f8;
          --code-header-text: #334155;
          --code-border: #d7e0ea;
          --inline-code-bg: #ebf1f8;
          --inline-code-text: #0f4a8a;
          --katex-bg: #f8fafc;
          --typing-dot: #9ca3af;
        }
        #rcw-root.dark {
          ...
          --msg-bot-text: #e2ebff;
          --input-bg: #101b31;
          --header-bg: linear-gradient(135deg, #111b2f, #17345c);
          --header-text: #eaf1ff;
          --code-bg: #f4f7fb;
          --code-header-bg: #e8eef6;
          --code-header-text: #334155;
          --code-border: #b8c7d8;
          --inline-code-bg: #0d203f;
          --inline-code-text: #92c5ff;
          --katex-bg: #101b31;
          --typing-dot: #4b5563;
        }
        ...
        .rcw-header-title { font-size: 14px; font-weight: 700; letter-spacing: -.01em; }
        .rcw-header-sub   { font-size: 11px; opacity: .88; display: flex; align-items: center; gap: 5px; }
        .rcw-header-sep { opacity: .65; }
        .rcw-header-subtitle {
          white-space: nowrap;
          overflow: hidden;
        ...
        .rcw-bubble h1,.rcw-bubble h2,.rcw-bubble h3 { font-weight: 700; margin: 14px 0 8px; line-height: 1.35; }
        .rcw-bubble h1 { font-size: 17px; }
        .rcw-bubble h2 { font-size: 15px; }
        .rcw-bubble h3 { font-size: 14px; }
        .rcw-bubble p  { margin: 9px 0; }
        .rcw-bubble p:first-child { margin-top: 0; }
        .rcw-bubble p:last-child  { margin-bottom: 0; }
        .rcw-bubble ul,.rcw-bubble ol { padding-left: 20px; margin: 8px 0; }
        ...
        .rcw-code-header {
          ...
          padding: 6px 12px;
          font-size: 11px; color: var(--code-header-text);
        }
        ...
        .rcw-copy-btn:hover { background: color-mix(in srgb, var(--p) 12%, transparent); color: var(--p); }
    ...
    _buildDOM() {
      ...
      this._root.innerHTML = `
        ...
        <div id="rcw-window">
          ...
          <div id="rcw-header">
            ...
            <div class="rcw-header-info">
              ...
              <div class="rcw-header-sub">
                ...
                <span id="rcw-status-text">Online</span>
                <span class="rcw-header-sep">•</span>
                <span class="rcw-header-subtitle">${this.cfg.subtitle}</span>
              </div>
            </div>
          ...
          <div id="rcw-input-area">
            ...
            <button id="rcw-send" aria-label="Gửi tin nhắn">
              <svg viewBox="0 0 24 24" width="18" height="18" aria-hidden="true" focusable="false">
                <path fill="currentColor" d="M3.4 20.2 21.6 12 3.4 3.8l1.5 6.6 8.2 1.6-8.2 1.6-1.5 6.6Z"/>
              </svg>
            </button>
          </div>
        </div>
      ...
      // Show welcome
      this._appendBotBubble(this.cfg.welcomeMessage, true);
    }
    ...
    async _send() {
      ...
      try {
        ...
        const flushStreamingText = () => {
          ...
          rawHolder.textContent = raw;
          this._scrollBottom();
          hasPendingChunk = false;
        };

        const scheduleFlush = () => {
          if (this._streamFramePending) return;
          this._streamFramePending = true;
    ...
    _renderBotBubble(bubble, raw, opts = {}) {
      bubble._rcwRaw = raw;
      ...
      if (!opts.skipScroll) this._scrollBottom();
    }
    ...
    _protectMath(text) {
      ...
      const protected2 = protected1.replace(/\\\[([\s\S]*?)\\\]/g, match => {
        const idx = store.length;
        store.push(match);
        return `RCWMATH_BLOCK_${idx}_END`;
      });
    ...
    _render(raw) {
      ...
      const { text: safeText, store } = this._protectMath(raw);
      ...
      renderer.code = (code, lang) => {
        ...
        const actualCode = token ? token.text : code;
        const actualLang = token ? token.lang : lang;
        const safeLang = actualLang ? this._esc(actualLang) : 'text';
        const safeCode = this._esc(actualCode ?? '');
        return `
      ...
      const restoredHtml = this._restoreMath(rawHtml, store);
    ...
    _renderMath(container) {
      ...
      try {
        ...
        container.querySelectorAll('.katex-display').forEach(node => {
          if (node.closest('.rcw-math-block')) return;
          const wrapper = document.createElement('div');
          wrapper.className = 'rcw-math-block';

          const copyBtn = document.createElement('button');
          copyBtn.type = 'button';
          copyBtn.className = 'rcw-copy-btn rcw-math-copy';
          copyBtn.textContent = '📋 Copy';

          // Preserve original TeX expression from KaTeX annotation when available.
          const annotation = node.querySelector('annotation[encoding="application/x-tex"]');
          const tex = annotation?.textContent?.trim() || node.textContent.trim();
          copyBtn.dataset.math = tex;
    ...
    _attachCopyBtns(container) {
      container.querySelectorAll('.rcw-copy-btn').forEach(btn => {
        btn.addEventListener('click', async () => {
          const payload = btn.dataset.code || btn.dataset.math || '';
          try {
